...
|---|---|---|
| "Not secure" / `NET::ERR_CERT_AUTHORITY_INVALID` | DNS not pointing at Vercel yet, so no cert can be issued | [Cert error](#cert-error-not-secure-on-your-domain) |
| `ERR_TOO_MANY_REDIRECTS` (redirect loop) | Cloudflare SSL mode = Flexible, or double redirect rules | [Redirect loop](#redirect-loop-err_too_many_redirects) |
| Domain says "Pending Verification" / "Invalid Configuration" | CNAME/A value wrong, or DNS not propagated | [Stuck pending](#stuck-pending-invalid-configuration-or-pending-verification) |
| Site loads but some assets blocked | Mixed content (HTTP resource on HTTPS page) | [Mixed content](#mixed-content-site-loads-but-assets-are-blocked) |

## Cert error: "not secure" on your domain
...
2. Look at the status badge next to your domain.
   ...
   - **"Invalid Configuration"** or **"Pending"** → stop here and fix DNS (see [Stuck pending](#stuck-pending-invalid-configuration-or-pending-verification) below). The cert error is a *symptom* of bad DNS, not a separate problem.
...
## Stuck pending: "Invalid Configuration" or "Pending Verification"
...
## Mixed content: site loads but assets are blocked
...
Almost always a DNS edit or a Cloudflare setting someone touched. Re-read the [redirect loop](#redirect-loop-err_too_many_redirects) and [stuck pending](#stuck-pending-invalid-configuration-or-pending-verification) sections; one small toggle is the usual culprit. Vercel doesn't silently revoke working certs.
